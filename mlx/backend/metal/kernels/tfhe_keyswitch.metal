...
// ============================================================================
// TFHE Parameters for Key Switching
// ============================================================================

constant uint N_DEFAULT = 1024;       // Large LWE dimension (from GLWE)
constant uint N_SMALL_DEFAULT = 630;  // Small LWE dimension (original)
constant uint KS_LEVELS = 4;          // Key switching decomposition levels
constant uint KS_BASE_LOG = 4;        // Key switching base log (Bg = 16)

// ============================================================================
// Key Switch Parameters Structure
// ============================================================================
...
// Parallelizes over input coefficients with atomic accumulation
// Better for large N_in with sufficient parallelism
kernel void keyswitch_parallel_input(
    device const ulong* lwe_in_a [[buffer(0)]],
    device const ulong* lwe_in_b [[buffer(1)]],
    device const ulong* ksk [[buffer(2)]],
    device atomic_ulong* lwe_out [[buffer(3)]],  // Atomic for accumulation
    constant KeySwitchParams& params [[buffer(4)]],
    ...
    if (input_idx >= N_in || output_idx > N_out || batch_idx >= params.batch) return;
    ...
    // Note: This is simplified; real atomic modular add needs CAS loop
    if (contrib != 0) {
        atomic_fetch_add_explicit(&lwe_out[batch_idx * (N_out + 1) + output_idx], 
                                   contrib, 
